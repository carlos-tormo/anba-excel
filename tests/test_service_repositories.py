...
class ServiceRepositoryBoundaryTests(unittest.TestCase):
    ...
    def test_player_happiness_repository_owns_import_sql(self) -> None:
        repository_source = inspect.getsource(PlayerHappinessRepository)
        service_source = inspect.getsource(PlayerHappinessService)
        self.assertIn("player_happiness_events", repository_source)
        self.assertIn("UPDATE player_profiles", repository_source)
        self.assertNotIn(".execute(", service_source)
