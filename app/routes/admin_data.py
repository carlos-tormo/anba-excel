...
def preview_player_rating_import(handler: Any, _parsed: ParseResult, payload: Optional[Dict[str, Any]]) -> Optional[RouteResponse]:
    payload = payload or {}
    if not _require_admin_post(handler) or not handler._authorize("admin.import.write"):
        return
    ...
    try:
        result = handler.app.player_rating_import.preview(json_text or payload.get("data") or payload)
    except ValueError as err:
        return json_response(200, {
            "ok": False,
            "errors": [{"line": None, "message": str(err) or "invalid_rating_import"}],
            "records": [],
            "summary": {
...
def apply_player_rating_import(handler: Any, _parsed: ParseResult, payload: Optional[Dict[str, Any]]) -> Optional[RouteResponse]:
    payload = payload or {}
    if not _require_admin_post(handler) or not handler._authorize("admin.import.write"):
        return
    ...
    if not isinstance(records, list) or not records:
        return error_response(400, "records_required")
    try:
        backup = handler.app.maintenance.create_verified_backup("pre_player_rating_import")
    except Exception as err:
        return error_response(500, "pre_import_backup_failed", detail=str(err))
    try:
        result = handler.app.player_rating_import.apply(records)
    except ValueError as err:
        if str(err) in {
            "records_required",
            "invalid_records",
            "invalid_rating_import_target",
            "duplicate_rating_import_target",
            "rating_import_target_changed",
        }:
            return error_response(400, str(err))
        raise
    details = {
    ...
    handler._log_admin_action(
        ...
        None,
        details,
        command_id=result.get("command_id") or f"player-ratings:import:{backup.get('id')}",
